...
def write_cvs(data):
    f = open(sys.argv[1], "w")
    wr = csv.writer(f, encoding="utf-8", quoting=csv.QUOTE_ALL)
    for d in data:
        for i in d["items"]:
            random_date = radar.random_datetime(start=FROM_DATE, stop=TO_DATE)
            date_ = random_date.strftime("%x")
            time_ = random_date.strftime("%X")
            wr.writerow((i["item_name"], d['cat_name'], i['item_price'], date_, time_, i['item_transaction']))
    f.close()
